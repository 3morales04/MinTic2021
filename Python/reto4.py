...
def ubicar_zonas_cercanas():
    ...
    if len(coordenadas) > 0:
        ...
        if opc >= 1 and opc <= 3:
            ...
            punto_seleccionado: list = coordenadas[opc-1]
            # Lista para almacenar todas las distancias y las coordenadas de los puntos de conexión
            # [ (distancia, [lat, lng, cant_personas]), (distancia2, [..])... ]
            distancias: list = []
            for u in ubicaciones:
                punto_ini: tuple = tuple(punto_seleccionado)
                punto_fin: tuple = tuple(u)
                # Calcula la distancia entre los puntos y los almacena en la lista
                distancia = calcular_distancia((punto_ini, punto_fin))
                item: tuple = ( distancia, punto_fin)
...
def mostrar_indicaciones(zona: tuple, ubicacion_Actual: tuple):
    ...
    punto_cardenal_2: str = "" #oriente u occidente
    if lat > y:
    ...
    print(f"Para llegar a la zona wifi debe dirigirse primero al {punto_cardenal_1} y luego hacia el {punto_cardenal_2}")
    #Regresar al menú
